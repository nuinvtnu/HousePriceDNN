...
for model_name, model in models.items():
    ...
    # Danh sách để lưu lại dict kết quả trả về của từng fold
    fold_metrics_list = []

    for fold, (train_idx, test_idx) in enumerate(kf.split(X, y)):
        # Trích xuất dữ liệu fold bằng .iloc (Giữ nguyên kiểu Pandas DataFrame/Series)
        ...
        # Gọi hàm save_result cho từng fold một để đảm bảo đúng định dạng Pandas mong muốn
        # Đặt tên model kèm số fold để tránh ghi đè file (ví dụ: LinearRegression_Fold1)
        ...
        # Lưu kết quả metric (ví dụ: R2, RMSE...) của fold này vào list
        if metrics_fold is not None:
            fold_metrics_list.append(metrics_fold)

        print(f"  - Fold {fold + 1} finished.")

    # Tính trung bình cộng các chỉ số sau 5 fold cho mô hình này
    if fold_metrics_list:
        # Chuyển list các dict thành DataFrame để dùng hàm .mean()
        df_fold_metrics = pd.DataFrame(fold_metrics_list)

        # Tính toán trung bình (Chỉ tính trên các cột có giá trị số)
        mean_metrics = df_fold_metrics.mean(numeric_only=True).to_dict()

        # Thêm lại tên mô hình vào dict kết quả trung bình
...
# Đẩy cột Model_Name lên đầu bảng nếu có dữ liệu
